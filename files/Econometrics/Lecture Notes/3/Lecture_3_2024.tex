...
\[E[\mathbf{Xy}]=E[\mathbf{X'X}]\pmb{\beta}\]
...
be expected to behave when it is applied to a sample of data.
\end{frame}
...
\(\mathbf{X}\), the unconditional mean is also \(\pmb{\beta}\).
...
\[\mathbf{y}=\mathbf{X}\pmb{\beta}+\gamma z + \pmb{\varepsilon}\]
...
\(\pmb{\beta}\), which would distort \(\sigma^2\).
...
Thus,
\(\mathbf{b}|\mathbf{X} \sim N(\pmb{\beta},\sigma^2(\mathbf{X}'\mathbf{X})^{-1})\)
\end{frame}
...
\item
  \textbf{A5a}:
  \((\mathbf{x}_i,\mathbf{\varepsilon}_i), i=1,\cdots, n\), is a
  sequence of independent, identically distributed \emph{observations}.
\item
  \textbf{A2'}:
...
\(\text{plim }\left(\frac{\mathbf{X}'\pmb{\varepsilon}}{n}\right)\):
...
\(E[\mathbf{x}\pmb{\varepsilon}]\), which is zero (why?).
...
the \emph{limiting} distribution of \(\sqrt{n}(\mathbf{b}-\pmb{\beta})\)
...
\(\text{plim } f(\mathbf{b})=f(\pmb{\beta})\) and the asymptotic
covariance matrix is
\[\mathbf{\Gamma}[Asy.Var(\mathbf{b}-\pmb{\beta})]\mathbf{\Gamma}'\].
...
Coefficients}
\protect\hypertarget{confidence-interval-for-a-linear-combination-of-coefficients}{}
