...
From these results we can find that $$E[\mathbf{Xy}]=E[\mathbf{X'X}]\pmb{\beta}$$
...
Notice that this is the sample counterpart of the population condition $E[\mathbf{Xy}]=E[\mathbf{X'X}]\pmb{\beta}$.
...
The **properties** of that estimator are descriptions of how it can be expected to behave when it is applied to a sample of data.
...
$$E[\mathbf{b}|\mathbf{X}]=\pmb{\beta}$$

Moreover, 

$$E[\mathbf{b}]=E_x[E[\mathbf{b}|\mathbf{X}]]=E_x[\pmb{\beta}]=\pmb{\beta}$$

\vfill

This is to say that the Least Squares estimator has expectation $\pmb{\beta}$. 

\vfill

Moreover, when we average this over the possible values of $\mathbf{X}$, the unconditional mean is also $\pmb{\beta}$.

\vfill

### Omitted Variable Bias (OVB)

Suppose the true population model is given by $$\mathbf{y}=\mathbf{X}\pmb{\beta}+\gamma z + \pmb{\varepsilon}$$
...
$$\begin{split}
\mathbf{b} & =(\mathbf{X}'\mathbf{X})^{-1}\mathbf{X}'\mathbf{y}=(\mathbf{X}'\mathbf{X})^{-1}\mathbf{X}'(\mathbf{X}\pmb{\beta}+\gamma z+\pmb{\varepsilon}) \\
 & = \pmb{\beta}+(\mathbf{X}'\mathbf{X})^{-1}\mathbf{X}'\gamma z+(\mathbf{X}'\mathbf{X})^{-1}\mathbf{X}'\mathbf{\varepsilon}
 \end{split}$$
...
$$\begin{split}
E[\mathbf{b}|\mathbf{X},z] & =\pmb{\beta}+(\mathbf{X}'\mathbf{X})^{-1}\mathbf{X}' \gamma z\\
 & = \pmb{\beta} + \mathbf{p}_{X.z}\gamma,
\end{split}$$
...
However, we would also need to estimate $K$ parameters $\pmb{\beta}$, which would distort $\sigma^2$.
...
$$\hat{Var(\mathbf{b}|X)}=Est.Var[\mathbf{b}|X]=s^2(\mathbf{X'X})^{-1}$$
...
$\mathbf{b}=\pmb{\beta}+(\mathbf{X}'\mathbf{X})^{-1}\mathbf{X}'\mathbf{\varepsilon}$
...
Thus, $\mathbf{b}|\mathbf{X} \sim N(\pmb{\beta},\sigma^2(\mathbf{X}'\mathbf{X})^{-1})$
...
We will show that the LS estimator $\mathbf{b}$ is consistent, that is, $\text{plim } \mathbf{b}= \pmb{\beta}$.

\vfill

We can write $\mathbf{b}$ as

$$\mathbf{b}=\pmb{\beta}+\left(\frac{\mathbf{X}'\mathbf{X}}{n}\right)^{-1}\left(\frac{\mathbf{X}'\pmb{\varepsilon}}{n}\right)$$
...
$$\text{plim } \mathbf{b}=\pmb{\beta}+\mathbf{Q}^{-1}\text{plim }\left(\frac{\mathbf{X}'\pmb{\varepsilon}}{n}\right)$$

To find $\text{plim } \mathbf{b}$, we need to check $\text{plim }\left(\frac{\mathbf{X}'\pmb{\varepsilon}}{n}\right)$:
...
$$\frac{\mathbf{X}'\pmb{\varepsilon}}{n}=\frac{1}{n}\sum_{i=1}^n{\mathbf{x}_i\varepsilon_i}$$

converges in probability to its expected value, $E[\mathbf{x}\pmb{\varepsilon}]$, which is zero (why?).
...
Thus, $\text{plim } \mathbf{b}=\pmb{\beta}$.
...
$$\sqrt{n}(\mathbf{b}-\pmb{\beta})=\left(\frac{\mathbf{X}'\mathbf{X}}{n}\right)^{-1}\left(\frac{1}{\sqrt{n}}\right)\mathbf{X}'\pmb{\varepsilon}$$
\vfill


As we know that $\text{plim } \left(\frac{\mathbf{X}'\mathbf{X}}{n}\right)^{-1}=\mathbf{Q}^{-1}$, the *limiting* distribution of $\sqrt{n}(\mathbf{b}-\pmb{\beta})$ is the same as the limiting distribution of 

$$\mathbf{Q}^{-1}\frac{1}{\sqrt{n}}\mathbf{X}'\pmb{\varepsilon}$$
...
These cases extend beyond the reach of the Gauss-Marvok theorem.
...
$$\text{plim } f(\mathbf{b})= f(\text{plim } \mathbf{b})=f(\pmb{\beta})$$

and

$$\text{plim } \mathbf{C}(\mathbf{b})=\frac{\partial f(\pmb{\beta})}{\partial \pmb{\beta}'}=\mathbf{\Gamma}$$

### Asymptotic Distribution of a Function of $\mathbf{b}$: The Delta Method

Using a linear Taylor series approach, we expand this set of functions in the approximation

$$f(\mathbf{b})= f(\pmb{\beta})+\mathbf{\Gamma}\times (\mathbf{b}-\pmb{\beta})+\text{higher order terms}$$

\vfill

The higher order terms become negligible in large samples if $\text{plim } \mathbf{b}=\pmb{\beta}$.
...
The mean of the asymptotic distribution is $\text{plim } f(\mathbf{b})=f(\pmb{\beta})$ and the asymptotic covariance matrix is $$\mathbf{\Gamma}[Asy.Var(\mathbf{b}-\pmb{\beta})]\mathbf{\Gamma}'$$.
...
$$b_k|\mathbf{X} \sim N(\mathbf{\beta}_k, \sigma^2 S^{kk})$$
...
$$y^0=\mathbf{x}^{0'}\pmb{\beta}+\varepsilon^0$$

\vfill

The prediction error is

$$e^0=\hat{y}^0-y^0=(\mathbf{b}-\pmb{\beta})'\mathbf{x}^0-\varepsilon^0$$
...
$$Var[e^0|\mathbf{X},\mathbf{x^0}]=Var[(\mathbf{b}-\pmb{\beta})'\mathbf{x^0}-\varepsilon^0|\mathbf{X},\mathbf{x^0}]=\sigma^2+\mathbf{x^0}'[\sigma^2(\mathbf{X'X})^{-1}]\mathbf{x^0}$$
