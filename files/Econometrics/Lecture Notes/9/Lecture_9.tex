...
\[\begin{split} y_{it} = & \mathbf{x}_{it}'\beta+\mathbf{z}_i'\alpha+\varepsilon_{it}\\
 = & \mathbf{x}_{it}'\beta+ c_i+\varepsilon_{it} \end{split}\]
...
The heterogeneity, or individual effect, is \(\mathbf{z}_i'\alpha\)
where \(\mathbf{z}_i\) contains a constant term and a set of individual
or group-specific variables, which may be observed, such as race, sex,
location, and so on; or unobserved, such as family specific
characteristics, individual heterogeneity in skill or preferences, and
so on, all of which are taken to be constant over time \(t\).
...
\[\mathbf{\beta}=\partial E[y_{it}|\mathbf{x}_{it}]/\partial \mathbf{x}_{it} \]
...
\item
  ...
  \(\mathbf{\beta}\) is biased and inconsistent (why?). However, the
  model \[y_{it}=\mathbf{x}_{it}'\beta+ \alpha_i+\varepsilon_{it},\]
...
\[\begin{split} \mathbf{b} = & (\mathbf{X'X})^{-1}\mathbf{X'y} \\
 = & \left[\sum_{i=1}^n{\mathbf{X_i'X_i}} \right]^{-1}\sum_{i=1}^n{\mathbf{X}_i'\mathbf{w}_i}  \end{split}\]
...
\[Asy.Var[\mathbf{b}]=\frac{1}{n}\text{plim }\left[\frac{1}{n} \sum_{i=1}^n{\mathbf{X_i'X_i}} \right]^{-1}\text{plim }\left[\sum_{i=1}^n{X_i '\mathbf{\Omega}_iX_i} \right]\text{plim }\left[\frac{1}{n} \sum_{i=1}^n{\mathbf{X_i'X_i}} \right]^{-1}\]
...
\[Est.Asy.Var[\mathbf{b}]=\frac{1}{n}\text{plim }\left[\frac{1}{n} \sum_{i=1}^n{\mathbf{X_i'X_i}} \right]^{-1}\text{plim }\left[\sum_{i=1}^n{X_i '\mathbf{\hat{w}\hat{w}}_i'X_i} \right]\text{plim }\left[\frac{1}{n} \sum_{i=1}^n{\mathbf{X_i'X_i}} \right]^{-1}\]
...
\begin{frame}{Bootstrapped Covariance Matrix}
\protect\hypertarget{bootstrapped-covariance-matrix}{}
Bootstrapping offers another approach to estimating an appropriate
...
To compute the \textbf{block bootstrap} estimator, we draw random
samples of \(N=n\) with \emph{repetition}. Then, we repeat the process
\(R\) times.

\vfill

After \(R\) repetitions, we compute the empirical variance of the \(R\)
replicates.
...
\[Est.Asy.Var[\mathbf{b}]=\frac{1}{R}\sum_{r=1}^R{(\mathbf{b_r-\bar{b}})(\mathbf{b_r-\bar{b}})'}\]
...
\[(1/T)\mathbf{i}'\mathbf{y}_i=(1/T)\mathbf{i'x_i\beta}+(1/T)\mathbf{i'w}_i\]

or

\[\bar{y}_i=\mathbf{\bar{x}}_i'\mathbf{\beta}+\bar{w}_i\]
...
Why might one want to use this estimator when the full data set is
available?
...
\[\mathbf{y}_i= \mathbf{x_i\beta}+c_i\mathbf{i}+\mathbf{w}_i,\]
...
\begin{frame}{Robust Estimation Using Group Means}
\protect\hypertarget{robust-estimation-using-group-means-2}{}
A common specification is Mundlak's (1978), where we employ the
projection of \(c_i\) on the group means

\[c_i|\mathbf{X}_i=\mathbf{\bar{x}}_i'\mathbf{\gamma}+\nu_i\]

Then,

\[\begin{split} y_{it} = & \mathbf{x}_{it}'\mathbf{\beta}+c_i+\varepsilon_{it} \\
 = & \mathbf{x}_{it}'\mathbf{\beta} + \mathbf{\bar{x}}_i'\mathbf{\gamma}+[\nu_i + \varepsilon_{it}] \\
 = & \mathbf{x}_{it}'\mathbf{\beta} + \mathbf{\bar{x}}_i'\mathbf{\gamma}+u_{it},
\end{split}\]

where, by construction, \(Cov[u_{it},\mathbf{\bar{x}}_i]=0\).

\vfill

The implication is that the group means estimator estimates not
\(\beta\), but \(\beta+\gamma\).

\vfill

Averaging the observations in the group collects the entire set of
effects, observed and latent, in the group means.
...
\[y_{it} = c_i+ \mathbf{x}_{it}'\mathbf{\beta}+\varepsilon_{it}\]

which implies the first differences equation

\[\Delta y_{it} = \Delta c_i + (\Delta \mathbf{x}_{it})'\mathbf{\beta} + \Delta \varepsilon_{it} \]

or

\[\begin{split} \Delta y_{it} = &  (\Delta \mathbf{x}_{it})'\mathbf{\beta} + \varepsilon_{it} - \varepsilon_{i,t-1}\\
 = & (\Delta \mathbf{x}_{it})'\mathbf{\beta} + u_{it}
\end{split}\]
\end{frame}
...
However, first differencing might be useful when we have a setting
involving two time periods, before and after a treatment.

\vfill
\end{frame}

\begin{frame}{Estimating Treatment Effects with First Differencing}
\protect\hypertarget{estimating-treatment-effects-with-first-differencing}{}
Consider the model

\[y_{it}=c_i + \mathbf{x}_{it}'\mathbf{\beta}+\theta S_{it}+\varepsilon_{it},\]
where \(t=1,2\), and \(S_{it}=0\) is period 1 and \(S_{it}=1\) in period
2.

\vfill

The treatment effect would be \(E[\Delta y_i|\Delta \mathbf{x}_i=0]\).
So, taking the first difference, we find

\[\Delta y_i = \theta+(\Delta \mathbf{x}_{it})'\mathbf{\beta}+u_i\] For
\(\Delta \mathbf{x}_i=0\), the treatment effect is \(\theta\).
...
\[\ddot{y}=\mathbf{x}_{it}'\mathbf{\beta}+\ddot{\varepsilon}_{it}\]
...
\[S_{xx}^{between}=\sum_{i=1}^n{T(\mathbf{x}_{i}-\bar{\bar{\mathbf{x}}})(\mathbf{x}_{i}-\bar{\bar{\mathbf{x}}})'} \text{ and } S_{xy}^{between}=\sum_{i=1}^n{T(\mathbf{x}_{i}-\bar{\bar{\mathbf{x}}})(\mathbf{y}_{i}-\bar{\bar{\mathbf{y}}})'}\]
...
\[\mathbf{y}_i=\mathbf{i}\alpha_i+ \mathbf{x}_{it}'\beta+\mathbf{\varepsilon}_{i}\]

We can represent the model above as

\[\mathbf{y}_i=\mathbf{D\alpha}+ \mathbf{X\beta}+\varepsilon\] where
\(D=[\mathbf{d}_1,\mathbf{d}_2],\ldots,\mathbf{d}_n]\), and \(d_i\) is a
dummy vector indicating the \(i\)th unit.
...
\[\mathbf{D'Da}+\mathbf{D'X}\mathbf{b}_{LSDV} \text{ or } \mathbf{a}=[\mathbf{D'D}]^{-1}\mathbf{D}'(\mathbf{y}-\mathbf{D'X}\mathbf{b}_{LSDV})\]
...
\begin{frame}{The LSDV Model}
\protect\hypertarget{the-lsdv-model-5}{}
For the individual effects,
\(a_i=\bar{y}_i-\bar{\mathbf{x}}_i'\mathbf{b}_{LSDV}\),
...
Notice that \(Asy.Var[a_i]\) does not converge to zero as \(n\) grows.
Thus, the constant term estimators in the fixed effects model are
\textbf{not} consistent estimators of \(\alpha_i\).

\begin{itemize}
\tightlist
\item
  Each \(\alpha_i\) is estimated using only \(T\) observations.
\end{itemize}

\vfill

Moreover, notice that \textbf{all} time-invariant variables will be
absorbed by the model (a major drawback).

\vfill
\end{frame}

\begin{frame}{Testing for the Significance of the Group Effect}
\protect\hypertarget{testing-for-the-significance-of-the-group-effect}{}
If we are interested in testing for the presence of heterogeneity across
groups, the, we can test the hypothesis that the constant terms are all
equal with an \(F\) test.

\vfill

Under the null hypothesis of equality, the efficient estimator is the
...
\[F(n-1,nT-n-K)=\frac{(R_{LSDV}^2-R^2_{pooled})/n-1}{(1-R_{LSDV}^2)/nT-n-K}\]
...
We can describe a generalized differences-in-differences approach based
on the model above (the infamous Two-Way Fixed Effects)
...
might be appropriate the model the individual-specific constant terms as
...
\[\mathbf{\eta}_i=[\mathbf{\eta}_{i1},\mathbf{\eta}_{i2},\ldots,\mathbf{\eta}_{iT}]'\]
...
\(\mathbf{\Sigma}=E[\mathbf{\eta}_{i}\mathbf{\eta}_{i}'|\mathbf{X}_i]\)
...
\item
  Group means (betweem group) estimator
\end{enumerate}
...
\[\mathbf{\hat{\beta}}= (\mathbf{X'\Omega^{-1}X})^{-1}\mathbf{X'\Omega^{-1}y}=  \left[\sum_{i=1}^n{\mathbf{X_i'\Sigma^{-1} X_i}} \right]^{-1}\left[\sum_{i=1}^n{\mathbf{X_i'\Sigma^{-1} y_i}} \right]\]

As before, to compute \(\mathbf{\hat{\beta}}\), w transform the data and
use OLS with the transformed data.

\vfill

This requires
\(\Omega^{-1/2}=[\mathbf{I}_n \otimes \mathbf{\Sigma}]^{-1/2}=\mathbf{I}_n \otimes \mathbf{\Sigma}^{-1/2}\)
...
where
\(\theta=1-\frac{\sigma^2_\varepsilon}{\sqrt{\sigma^2_\varepsilon+T\sigma_u^2}}\).
...
Notice that \(\bar{e}_i=0\) in the LSDV model, but we carry it out to
stress that we are actually estimating \(E[\varepsilon_{it}]\).
...
\[\hat{\hat{\mathbf{\beta}}}=\mathbf{\beta}+\left[\sum_{i=1}^n{\mathbf{X_i'\hat{\Sigma}^{-1} X_i}} \right]^{-1}\left[\sum_{i=1}^n{\mathbf{X_i'\hat{\Sigma}^{-1} \varepsilon}_i} \right]\]

Following the approach used in earlier applications, the estimated
robust covariance matrix would be

\[Est.Asy.Var[b]=\left[\sum_{i=1}^n{\mathbf{X_i'\hat{\Sigma}^{-1} X_i}} \right]^{-1}\left[\sum_{i=1}^n{\left(\mathbf{X_i'\hat{\Sigma}^{-1} e}_i\right)\left(\mathbf{X_i'\hat{\Sigma}^{-1} e}_i\right)'} \right]\left[\sum_{i=1}^n{\mathbf{X_i'\hat{\Sigma}^{-1} X_i}} \right]^{-1}\]
\end{frame}
...
\(\hat{\mathbf{\beta}}_{pooled}\) are consistent, so the two estimates
...
\[\mathbf{W}=\chi^2[K-1]=[\mathbf{b}_{LSDV}-\hat{\mathbf{\beta}}_{pooled}]'[Var(\mathbf{b}_{LSDV}-\hat{\mathbf{\beta}}_{pooled})]^{-1}[\mathbf{b}_{LSDV}-\hat{\mathbf{\beta}}_{pooled}]\]
...
\includegraphics{black_results.png}
\end{frame}
...
Differences-in-Differences do not nee to be \emph{only} differences
...
\protect\hypertarget{sources-of-data}{}
...
The intensity of the building program in the birth region of each person
in the sample.
...
\begin{frame}{Look at the Average Years of Schooling}
\protect\hypertarget{look-at-the-average-years-of-schooling-1}{}
\includegraphics{duflo_simple_comparison2.png}
\end{frame}
...
Cheng and Hoekstra (2013). \emph{Does Strengthening Self-Defense Law
Deter Crime or Escalate Violence?: Evidence from Expansions to Castle
Doctrine.} \textbf{Journal of Human Resources}
\end{frame}

\begin{frame}{Summary}
\protect\hypertarget{summary}{}
Cheng and Hoekstra (2013) are interested in whether expansions to
``castle doctrine statutes'' at the state level increase or decrease gun
violence

\vfill

Prior to these expansions, English common law principle required ``duty
to retreat'' before using lethal force against an assailant except when
the assailant is an intruder in the home
...
\item
  When intruders threatened the victim in the home, the duty to retreat
  was waived and lethal force in self-defense was allowed
\end{itemize}

\vfill
\end{frame}

\begin{frame}{Castle Doctrine Law Explained}
\protect\hypertarget{castle-doctrine-law-explained}{}
In 2005, Florida passed a law that expanded self-defense protections
beyond the house.

\vfill

From 2000 to 2010, 21 states explicitly put castle doctrine into
statute, and (more importantly) extended it to places outside the home.

\begin{itemize}
\tightlist
\item
  In other words, 21 states removed the duty to retreat in specified
  circumstances
\end{itemize}
...
\protect\hypertarget{economic-theory}{}
Workers supply legal or illegal labor

\begin{itemize}
\tightlist
\item
  Costs: foregone wages, risk of arrest, risk of conviction, penalties
...
Castle doctrine expansions lowered the (expected) cost of killing
someone in self-defense

\vfill

If people are rational, then lowering the price of lethal self-defense
should increase lethal homicides

\vfill
\end{frame}

\begin{frame}{Cheng and Hoekstra's Identification Strategy}
\protect\hypertarget{cheng-and-hoekstras-identification-strategy}{}
Research design:

\begin{itemize}
\tightlist
\item
  Estimate the state-level causal effect: what would've happened to
  these same states had they not passed the law?
\item
  Compare the changes in outcomes after castle doctrine law adoption to
  changes in the outcomes in other states in the same region of the
  country
\end{itemize}
...
\(CDL\) is a dummy variable equaling 1 if the state has a castle
doctrine law and zero otherwise.
...
\protect\hypertarget{region-by-year-fixed-effects}{}
Most of their specifications will include region dummies interacted with
year dummies or ``region-by-year fixed effects''

\vfill

Absent passing castle doctrine laws, outcomes in these 21 states would
have changed similar to other states in their same region.

\begin{itemize}
\tightlist
\item
  By including ``region-by-year fixed effects'', they are arguing that
  unobserved changes in crime are running ``parallel''to the treatment
  states within region over time
\item
  Need not hold across regions since the across region variation is not
  being used in this analysis due to the saturation of the model with
...
\begin{frame}{Testing the identification strategy}
\protect\hypertarget{testing-the-identification-strategy}{}
\includegraphics{homicides_florida.png}
\end{frame}

\begin{frame}{Testing the identification strategy}
\protect\hypertarget{testing-the-identification-strategy-1}{}
\includegraphics{homicides_all.png}
\end{frame}

\begin{frame}{Results}
\protect\hypertarget{results}{}
\includegraphics{homicide_result.png}
\end{frame}

\begin{frame}{Conclusions}
\protect\hypertarget{conclusions-1}{}
These laws do lead to an 8\% net increase in homicide rates, translating
to around 600 additional homicides per year across the 21 adopting
states
