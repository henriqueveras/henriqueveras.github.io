...
Panel data models can broadly be arranged as follows:
  ...
  1. **Pooled regression**: If $\mathbf{z}_i$ contains only a constant term, then the OLS estimator provides consistent and efficient estimates of the common $\alpha$ and slope vector $\mathbf{\beta}$.
  2. **Fixed effects**: If $\mathbf{z}_i$ is unobserved, but correlated with $\mathbf{x}_{it}$, then the OLS estimator of $\mathbf{\beta}$ is biased and inconsistent (why?). However, the model $$y_{it}=\mathbf{x}_{it}'\beta+ \alpha_i+\varepsilon_{it},$$ where $\alpha_i=\mathbf{z}_i'\alpha$ embodies all the observable effects and specifies an estimable conditional mean.
...
Throughout the analysis we consider assumptions A.1. through A.4., as well as the asymptotic conditions on the matrices $\mathbf{X'X}/n$ and $\mathbf{(X'\varepsilon)/n}$.
...
The implication is that the group means estimator estimates not $\beta$, but $\beta+\gamma$.
...
$$\Delta y_i = \theta+(\Delta \mathbf{x}_{it})'\mathbf{\beta}+u_i$$
For $\Delta \mathbf{x}_i=0$, the treatment effect is $\theta$.
...
Notice that $Asy.Var[a_i]$ does not converge to zero as $n$ grows. Thus, the constant term estimators in the fixed effects model are **not** consistent estimators of $\alpha_i$.
...
If we are interested in testing for the presence of heterogeneity across groups, the, we can test the hypothesis that the constant terms are all equal with an $F$ test.
...
As before, let's also represent the model in blocks of $T$ observations for group $i$, $\mathbf{y}_i$, $\mathbf{X}_i$, $u_i$, and $\mathbf{\varepsilon}_i$.
...
For the $T$ observations for unit $i$, let $\mathbf{\Sigma}=E[\mathbf{\eta}_{i}\mathbf{\eta}_{i}'|\mathbf{X}_i]$
...
Notice that we have also discussed four other methods that consistently estimates $\mathbf{\beta}$:
  ...
  4. Group means (betweem group) estimator
...
As before, to compute $\mathbf{\hat{\beta}}$, w transform the data and use OLS with the transformed data.

\vfill

This requires $\Omega^{-1/2}=[\mathbf{I}_n \otimes \mathbf{\Sigma}]^{-1/2}=\mathbf{I}_n \otimes \mathbf{\Sigma}^{-1/2}$
...
where $\theta=1-\frac{\sigma^2_\varepsilon}{\sqrt{\sigma^2_\varepsilon+T\sigma_u^2}}$.
...
We can, thus, estimate $\mathbf{\beta}$ by the LSDV estimator, which is consistent and unbiased in most cases, and use its residuals, 
...
Under $H_0$, both $\mathbf{b}_{LSDV}$ and $\hat{\mathbf{\beta}}_{pooled}$ are consistent, so the two estimates should not differ systematically.
...
Differences-in-Differences do not nee to be *only* differences between units over time
