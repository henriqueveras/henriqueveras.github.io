...
We further assume the following:
  ...
  - **A.I7**: $[\mathbf{x}_i,\mathbf{z}_i,\varepsilon_i], i=1,\cdots,n$, are an $i.i.d$ sequence of random variables.
  - **A.I8a**: $E[x_{ik}^2]=\mathbf{Q}_{\mathbf{xx},kk}<\infty$, a finite constant, $k=1,\cdots , K$.
  - **A.I8b**: $E[z_{il}^2]=\mathbf{Q}_{\mathbf{zz},ll}<\infty$, a finite constant, $l=1,\cdots , L$.
  - **A.I8c**: $E[z_{il} x_{ik}]=\mathbf{Q}_{\mathbf{zx},lk}<\infty$, a finite constant, $l=1,\cdots , L$, $k=1,\cdots K$.
...
$$y_i=x_{1i}\beta_1+x_{2i}\beta_2+\cdots+x_{Ki}\beta_K$$ with $cov(\mathbf{x}_k,\mathbf{\varepsilon})\neq \mathbf{0}$.
...
The IV estimator, based on instrument $\mathbf{z}$, proceeds from two conditions:
  ...
  2. **Exogeneity**: $E(\mathbf{\varepsilon}|\mathbf{z})=0$
...
Can you think about any reason for why this instrument might not satisfy this assumption?
  - One potential reason: geography might affect both historic mortality rates and present-day economic output.
...
Notice that, by construction, $\mathbf{x}$ and $\mathbf{w}_0$ are independent.
...
In comparing $\mathbf{b}_{LS}$ and $\mathbf{b}_{IV}$, we see that $\mathbf{b}_{LS}$ is unambiguously more efficient.
...
However, $\mathbf{b}_{IV}$ is robust, as it is consistent whether or not $\text{plim }(\mathbf{X'\varepsilon}/n)=0$.

\vfill

If the condition above holds (that is, $\gamma=0$), then IV is not needed so $\mathbf{b}_{IV}$ would be preferable.
...
Consider a comparison of the covariance matrices of $\mathbf{b}_{LS}$ and $\mathbf{b}_{IV}$ under the hypothesis that both estimators are consistent ($\text{plim }(\mathbf{X'\varepsilon}/n)=0$).
...
Hausman (1978) developed an alternative strategy. Under $H_0$, both $\mathbf{b}_{IV}$ and $\mathbf{b}_{LS}$ are consistent estimators of $\beta$.

\vfill

The suggestion is then to examine $\mathbf{d}=\mathbf{b}_{IV}-\mathbf{b}_{LS}$.
...
where $\hat{\mathbf{H}}^{-1}$ is the estimator of $\{Est.Asy.Var(\mathbf{b}_{IV})-Est.Asy.Var(\mathbf{b}_{LS})\}^{-1}$, $\sigma^2 \{[\mathbf{X'X}-\mathbf{X'M_ZX}]^{-1}-[\mathbf{X'X}]^{-1}\}^{-1}$.
...
When $L=K$, the system is exactly identified and there is only one solution to $\frac{1}{n}\sum{\mathbf{z}_ie_i}=0$.
...
However, the problem goes beyond precision of the estimates. A recent literature has been shown that 
...
Some variables simply have no observable counterpart: Education, ability, intelligence are examples.
...
Although we cannot observe ability, we can use an indicator, IQ, which can be correlated with the unobserved variable $Ability$:

$$IQ=\alpha_1 +\alpha_2 Ability+\nu$$

### Proxy Variables

We can insert one equation into the other to obtain the **reduced form** equation
...
Notice that for us to being able to estimate this equation, we require:
  ...
  1. $IQ$ not be related to the other variables of the model
  2. $\nu$ not be related to the other variables of the model
  3. $E[Earnings|Experience, Industry, Ability,IQ]=E[Earnings|Experience, Industry, Ability]$
