...
Consider an alternative approach: find the sample estimate of $\mathbf{r'}\pmb{\beta}$
...
Lagrange multiplier tests rely on the idea that testing whether $\lambda_*=0$ should be equivalent to testing $\mathbf{R}\pmb{\beta}-\mathbf{q}=\mathbf{0}$ (why?)
...
We use $\mathbf{b}$ as an estimator of $\pmb{\beta}$ and estimate $c(\pmb{\beta})$ with $c(\mathbf{b})$.

\vfill

According to the Slutsky theorem, if $\text{plim }b=\pmb{\beta}$, then $\text{plim }c(b)=c(\pmb{\beta})$.


## Structural Break and Parameter Variation

We are now interested in testing the hypothesis that some or all of the regression coefficients are different in different subsets of the data, which usually is applied to testing for **structural change**.

\vfill

A famous structural change test was proposed by Chow (1960). Let's discuss this test in the context of the U.S. gasoline market example.

\vfill

### Structural Break and Parameter Variation

![](/Users/henriquefonseca/Desktop/temp/Rmarkdown-practice/henriqueveras.github.io/files/Econometrics/Lecture Notes/5/figure_4.2.png)


### Different Parameter Vectors

We want to test whether the oil shock in 1973 changed the structure of the gasoline market.

\vfill

We proceed as follows: denote the first 21 years of the data in $\mathbf{y}$ and $\mathbf{X}$ as $\mathbf{y}_1$ and $\mathbf{X}_1$ and the remaining years as $\mathbf{y}_2$ and $\mathbf{X}_2$. 
...
Therefore, the total sum of squared residuals from this regression will be the sum of the two residual sums of squares from the two separate regressions:

$$\mathbf{e'e}=\mathbf{e_1'e_1}+\mathbf{e_2'e_2}$$

### The Restricted Least Squares
    
The restricted coefficient vector can be obtained by imposing a constraint on the least squares.

\vfill

Formally, we can write the restriction $\mathbf{\pmb{\beta}}_1=\mathbf{\pmb{\beta}}_2$ in the format $\mathbf{R\pmb{\beta}}=\mathbf{q}$, with $\mathbf{R}=[\mathbf{I}:-\mathbf{I}]$ and $\mathbf{q}=\mathbf{0}$.

\vfill

If the restriction holds, then the restricted estimator can be obtained simply by stacking the data and estimating a single regression
...
where $\mathbf{e}_*$ is the residuals vector from the restricted model.

\vfill

For this application, $J$ is the number of columns in $\mathbf{X}_2$ and the denominator degrees of freedom is $n_1+n_2-2K$:
