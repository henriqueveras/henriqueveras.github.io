...
$$\mathbf{y}=\textbf{x}_1\mathbf{\beta}_1+\textbf{x}_2\mathbf{\beta}_2+\cdots+\textbf{x}_k\mathbf{\beta}_k+\pmb{\varepsilon}$$

\vfill

Notice that the assumption is about the linearity in the parameters and the disturbance term rather than in the $\mathbf{x}$'s.
...
$$y_2=\beta_1 x_{12}+\beta_2x_{22}+\cdots\beta_kx_{k2}+\varepsilon_2$$

$$\vdots$$

$$y_n=\beta_1 x_{1n}+\beta_2x_{2n}+\cdots\beta_kx_{kn}+\varepsilon_n$$
...
$$\textbf{y}=\textbf{x}_1\mathbf{\beta}_1+\dots+\textbf{x}_K\mathbf{\beta}_K+\pmb{\varepsilon}$$


\begin{align}\textbf{y}=\mathbf{X}\pmb{\beta}+\pmb{\varepsilon}\end{align}

### A Note on Notation

We will use a boldface $\textbf{x}$ to denote a column or a row of $\textbf{X}$.

\vfill

Subscript $k$ will usually be used to denote columns (variables) and $i$ to denote observations.

\vfill

It will often be convenient to refer to a single observation as

$$y_i = \mathbf{x}_i'\pmb{\beta}+\varepsilon_i$$
...
**Assumption A3**: The disturbance is assumed to have conditional expected value zero at every observation: $E(\pmb{\varepsilon}|\mathbf{X})=\mathbf{0}$
...
Define the *residuals* from the estimated regression as $$e_i=y_i-\mathbf{x}'_i\mathbf{b}$$
...
### 
![](/Users/henriquefonseca/Desktop/temp/Rmarkdown-practice/henriqueveras.github.io/files/Econometrics/Lecture Notes/2/figure_3.1.png)
...
$$\mathbf{b}_0=(\mathbf{X}'\mathbf{X})^{-1}\mathbf{X}'\mathbf{y}$$

\vfill

For the solution $\mathbf{b}_0$ to minimize the sum of the squared residuals, the matrix $\frac{\partial^2 S(\mathbf{b})}{\partial \mathbf{b}^2}=2\mathbf{X}'\mathbf{X}$ must be positive definite.
...
Properties of the matrix $\mathbf{M}$:

  1. $\mathbf{M}$ is symmetric 
  2. $\mathbf{M}$ is idempotent
...
$$\mathbf{y}=\mathbf{X}\pmb{\beta}+\pmb{\varepsilon}=\mathbf{X}_1\pmb{\beta}_1+\mathbf{X}_2\pmb{\beta}_2+\pmb{\varepsilon}$$

How can we find the algebraic solution for $\mathbf{b}_2$? That is, what is the LS estimator of a given subset of parameters, $\pmb{\beta}_2$, in $\pmb{\beta}$?
...
This process is commonly called **partialing out** or **netting out** the effect of $\mathbf{X}_1$. For this reason, the coefficients in a multiple regression are often called the partial regression coefficients.
...
Example: Suppose we are interested again in the returns to education equation $$Income_i=\beta_1+\beta_2 educ_i + \beta_3 age_i + \beta_4 age^2_i+\varepsilon_i$$
...
Therefore, each column of $\mathbf{M}^0\mathbf{X}_2$ is the original variable, now in the form of deviations from the mean. This general result is summarized in the following corollary.
...
We will assume that there is a constant term in $\mathbf{X}$ so that the vectors of residuals $\mathbf{y}_*$ and $\mathbf{z}_*$ have zero sample means. then, the square of the partial correlation coefficient is
...
**Model 2**: $\ln Price = \gamma_1(1)+\gamma_2 \ln WH + \gamma_3 \ln W/H + u$
...
**Model 2**: $\ln Price = \gamma_1 z_1+\gamma_2 z_2 + \gamma_3 z_3 + u$
...
In our art appreciation example, what is the relationship between $\mathbf{b}$ and $\mathbf{d}$?
