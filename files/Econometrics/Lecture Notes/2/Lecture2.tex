...
Econometrics is concerned with \textbf{model building}.
...
Notice that the assumption is about the linearity in the parameters and
the disturbance term rather than in the \(\mathbf{x}\)'s.
\end{frame}
...
\protect\hypertarget{a-note-on-notation}{}
We will use a boldface \(\textbf{x}\) to denote a column or a row of
\(\textbf{X}\).

\vfill

Subscript \(k\) will usually be used to denote columns (variables) and
\(i\) to denote observations.
...
\begin{frame}{Full Rank}
\protect\hypertarget{full-rank}{}
...
\(E(\pmb{\varepsilon}|\mathbf{X})=\mathbf{0}\)
...
\includegraphics{/Users/henriquefonseca/Desktop/temp/Rmarkdown-practice/henriqueveras.github.io/files/Econometrics/Lecture Notes/2/figure_3.1.png}
...
For the solution \(\mathbf{b}_0\) to minimize the sum of the squared
...
It is important to note that none of these results need hold if the
regression does not contain a constant term.
...
This process is commonly called \textbf{partialing out} or
\textbf{netting out} the effect of \(\mathbf{X}_1\). For this reason,
the coefficients in a multiple regression are often called the partial
...
equation
\[Income_i=\beta_1+\beta_2 educ_i + \beta_3 age_i + \beta_4 age^2_i+\varepsilon_i\]
...
Therefore, each column of \(\mathbf{M}^0\mathbf{X}_2\) is the original
...
We will assume that there is a constant term in \(\mathbf{X}\) so that
the vectors of residuals \(\mathbf{y}_*\) and \(\mathbf{z}_*\) have zero
sample means. then, the square of the partial correlation coefficient is
...
\textbf{Model 2}:
\(\ln Price = \gamma_1(1)+\gamma_2 \ln WH + \gamma_3 \ln W/H + u\)
\end{frame}
...
\textbf{Model 2}:
\(\ln Price = \gamma_1 z_1+\gamma_2 z_2 + \gamma_3 z_3 + u\)
