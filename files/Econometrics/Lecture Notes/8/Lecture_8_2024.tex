...
\begin{frame}{Generalized Least Squares (GLS)}
\protect\hypertarget{generalized-least-squares-gls-2}{}
...
The transformed regression is a computational device, not the model of
interest.
...
The typical problem involves a small set of parameters \(\pmb{\alpha}\)
such that \(\mathbf{\Omega}=\mathbf{\Omega}(\pmb{\alpha})\)
...
Under Heteroskedasticity, \(\mathbf{\Omega}^{-1}\) is a diagonal matrix
whose \(i\)th diagonal element is \(1/\omega_i\).
...
If we assume \(\sigma_i^2=\sigma^2x_{ik}^2,\) the transformed regression
model for GLS is
...
\(\varepsilon^2 \geq 0\) cannot be normally distributed.
