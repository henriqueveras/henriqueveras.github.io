...
If we pre-multiply $\mathbf{y}=\mathbf{X\beta}+\mathbf{\varepsilon}$ by $P$, we obtain
...
The typical problem involves a small set of parameters $\mathbf{\alpha}$ such that $\mathbf{\Omega}=\mathbf{\Omega}(\alpha)$
...
$\mathbf{\Omega}^{-1}$ is a diagonal matrix whose $i$th diagonal element is $1/\omega_i$.
...
Notice that we are relying on large sample properties since $\varepsilon\geq 0$ cannot be normally distributed.
